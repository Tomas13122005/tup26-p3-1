...
- Agrega productos al carrito y reserva stock.
- Quita productos del carrito y repone stock.
- Muestra el total.
- Confirma o cancela el carrito.
- Persiste los cambios del carrito y del stock dentro de transacciones.
...
## 4. SQLite con una ruta local simple

La versión actual usa una ruta literal:

```csharp
var dbPath = "30.1-carrito.sqlite";
```

La decisión es guardar la base SQLite en un archivo local, fácil de borrar y recrear en cada ejecución.

Ventajas para una clase o demo:

- Es fácil encontrar la base generada.
- Mantiene el ejemplo corto y fácil de leer.
- Evita incorporar configuración adicional antes de tiempo.
- Permite borrar y recrear el archivo sin afectar otras partes del proyecto.

Costo de esta decisión:

- La ubicación final depende del directorio de trabajo desde donde se ejecute el programa.
...
> Para un ejemplo chico, una ruta simple alcanza. Para producción, la ubicación de la base debería salir de configuración y no quedar fija en el código.
...
Pero un `enum` deja mejor preparada la evolución del sistema. Hoy hay tres estados:
...
- cómo se agrega o actualiza un ítem
- cuándo hay que reservar o liberar stock
- cuándo hay que abrir una transacción y persistir cambios
...
La versión actual separa dos ideas:

- `NormalizarCodigo` unifica la identidad del producto.
- `Verificar` concentra precondiciones y mensajes de error.
...
AgregarProducto(carrito.Id, "NOTE-15", 1);
```
...
## 14. Confirmar o cancelar un carrito

Confirmar no borra el carrito ni crea otra estructura. Cambia su estado dentro de una transacción:
...
En esta versión, confirmar no descuenta stock porque la reserva ya ocurrió al agregar productos al carrito.

Cancelar también cambia el estado, pero además libera el stock reservado:
...
- Un producto debe tener stock disponible mayor a cero al crearse.
...
### 16.3 Concurrencia de stock

El ejemplo ya usa transacciones explícitas para agregar, quitar, confirmar y cancelar. En un sistema real faltaría resolver mejor la concurrencia entre usuarios:

- qué hacer si dos usuarios reservan el último ítem al mismo tiempo
- si conviene usar concurrencia optimista, bloqueos o reintentos
- cómo reportar conflictos de stock de manera amigable

### 16.4 Política de reservas

El ejemplo reserva stock al agregar al carrito y lo libera al quitar o cancelar. En un sistema real probablemente habría que definir además:

- vencimiento automático de carritos abandonados
- liberación diferida de stock
...
- sumar cantidad si el producto ya existe en el carrito
- eliminar parcialmente
- eliminar completamente
- impedir modificar carrito confirmado
- impedir modificar carrito cancelado
...
| `EstadoCarrito` como enum | Modela el ciclo de vida del carrito, incluyendo cancelación, mejor que booleanos. |
...
| Reserva inmediata de stock | Hace visible el impacto del carrito sobre inventario desde el momento de agregar productos. |
...
| Transacciones explícitas | Mantienen consistentes carrito y stock cuando una operación modifica varias entidades. |
